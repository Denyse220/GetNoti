...
<body>
  ...
  <div class="container-fluid">
    ...
    <div class="row">
      <div class="col-md-4">
        <h3 class="text-center text-info">ADD A NEW PROJECT</h3>
        <form action="pro.php" method="post" enctype="multipart/form-data" class="modal-body">
          <input type="hidden" name="id" value="<?= $id; ?>">
          <div class="form-group">
            <input type="text" name="name" value="<?= $name; ?>" class="form-control" placeholder="a project name" required>
          </div>
          <label>Project starting</label>
          <div class="form-group">
            <input type="date" name="sdate" value="<?= $sdate; ?>" class="form-control" placeholder="starting date" required>
          </div>
          <label>Project End date</label>
          <div class="form-group">
            <input type="date" name="edate" value="<?= $edate; ?>" class="form-control" placeholder="End date" required>
          </div>
      ...
      <div class="col-md-8">
        <?php
          $query = 'SELECT * FROM crudd';
          $stmt = $conn->prepare($query);
          $stmt->execute();
          $result = $stmt->get_result();
        ?>
        <h2 class="text-center text-info">LIST OF PROJECTS AND THEIR STATUSES</h2>
